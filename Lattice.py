...
class lattice(object):
    def __init__(self, xDim=100, yDim=0, D=1., kappa=0.5, sigma=2, dx=1., dt=1., initVal=0.5, noise=0.1):
        """
    ...
    def next(self):
        ...
        Perform one sweep.
        """
        self.phi = self.phi + self.D*self.dt*vc.lap2D(self.phi, self.dx) + self.dt*self.rho - self.kappa*self.dt*self.phi
        self.n += 1
        self.nList.append(self.n)
        self.avPhi.append(np.average(self.phi))
